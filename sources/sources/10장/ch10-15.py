"""프로그램 10.15: 세그먼트 트리에서 자료 원소의 갱신을 수행하는 프로그램"""
...
def update(i, start, end, idx, value, T):
    """ D[idx]의 값이 value로 갱신된 세그먼트 트리를 구축한다."""
    if start == end: 			# 잎 노드이면 자료 원소의 값을 갱신함
        T[i] = value
        return
    
    mid = (start + end) // 2            # 범위의 중앙 위치
    if start <= idx <= mid: 		# 왼쪽 부트리에 갱신할 노드가 있는 경우
        update(2*i+1, start, mid, idx, value, T)
    else: 				# 오른쪽 부트리에 갱신할 노드가 있는 경우
        update(2*i+2, mid+1, end, idx, value, T)
    T[i] = T[2*i+1] + T[2*i+2]		# 구간 합 갱신
...
D[1] = 8				    # D[1]의 값을 8로 변경함
update(0, 0, len(D)-1, 1, 8, T)		    # 자료 원소 갱신을 위한 호출
