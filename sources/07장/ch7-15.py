...
class Deque:
    ...
    def __init__(self):			# 공백 데크 생성
        self._head = self._DNode(0)     # 헤더 노드에 데크 크기 저장
    ...
    def _delete_node(self, p):	# 노드 p를 삭제/반환  (클래스 내부에서만 사용)
        if p is self._head:             # 헤더 노드 삭제 시도 시 예외 발생
